...
from backend.agents.fetcher.confluence.fetcher import fetcher_confluence_node
from backend.agents.fetcher.local.fetcher import fetcher_local_node
from backend.agents.fetcher.web.fetcher import fetcher_web_node
from backend.agents.index_log.index_log import index_log_node
from backend.agents.ingest.ingest import ingest_node
from backend.agents.normalizer.confluence.normalizer import normalizer_confluence_node
from backend.agents.normalizer.local.normalizer import normalizer_local_node
from backend.agents.normalizer.web.normalizer import normalizer_web_node
from backend.models.state import IngestState, SourceType
...
def _generate_source_id(url: str) -> str:
    ...
    raw_name = Path(url).stem if Path(url).name else url
    slug = re.sub(r"[^a-z0-9]+", "-", raw_name.lower()).strip("-")
    if not slug:
        slug = "source"
    digest = hashlib.sha1(url.encode("utf-8")).hexdigest()[:8]
    return f"{timestamp}-{slug[:40]}-{digest}"
