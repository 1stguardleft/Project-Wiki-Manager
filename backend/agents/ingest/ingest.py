...
중요 규칙:
- source에 있는 내용만 사용하세요. 외부 지식, 배경 설명, 일반론을 추가하지 마세요.
- 문서의 구조(섹션, 순서)를 최대한 유지하세요.
- 중복 제거, 문장 다듬기는 허용하지만 사실을 확장하거나 보태지 마세요.
...
중요 규칙:
- source에 있는 내용만 사용하세요.
- 외부 지식, 일반론, 정의, 예시를 추가하지 마세요.
- 여러 source가 같은 상위 주제를 다루면 한 페이지 안에서 정리하고, 해당 page에 꼭 필요한 정보만 쓰세요.
- 문장을 다듬거나 중복을 제거하는 것은 허용되지만 사실을 확장하면 안 됩니다.
- source에 없는 섹션 제목을 과하게 만들어내지 마세요.
...
"""
        ...
        prompt = f"""기존 wiki 페이지를 source 내용으로 갱신하세요.

중요 규칙:
- source에 있는 범위 안에서만 갱신하세요.
- 외부 지식, 일반론, 정의, 예시를 추가하지 마세요.
- 기존 내용을 유지하되, source와 직접 관련 없는 문장을 새로 덧붙이지 마세요.
- 같은 상위 주제를 설명하는 내용은 한 페이지 안에서 정리하고, 독립 개념이면 별도 페이지로 보내야 합니다.
- 문장을 정리하거나 중복을 제거하는 것은 허용되지만 사실을 확장하면 안 됩니다.
